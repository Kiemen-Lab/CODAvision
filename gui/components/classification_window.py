"""
Classification Window Component for CODAvision GUI

This module provides the window for visualizing and analyzing classification results
generated by the tissue segmentation models.
...
class MainWindowClassify(QtWidgets.QMainWindow):
    """
    Window for classifying and analyzing images with trained models.
    
    This class provides functionality for visualizing classification results,
    adjusting color maps, and performing quantitative analysis on classified images.
    """
    ...
    def select_imagedir(self):
        ...
        if folder_path:
            if os.path.isdir(folder_path):
                ...
                elif not (any([f.endswith(('.tif')) for f in os.listdir(folder_path)]) or any(
                        [f.endswith(('.png')) for f in os.listdir(folder_path)]) or any(
                        [f.endswith(('.jpg')) for f in os.listdir(folder_path)])):
                    QtWidgets.QMessageBox.warning(self, 'Warning',
...
class WorkerThread(QThread):
    ...
    def run(self):
        ...
        for index in range(self.list.count()):
            ...
            if self.is_running:
                classify_images(os.path.normpath(item.text()), os.path.join(self.train_fold, self.nm),
                                self.model_type,
                                disp=False)
            else:
            ...
            self.message_updated.emit(
                f'Changing colormap of images from path {index + 1}/{self.list.count()}: {image_path}')
            ...
            self.progress_updated.emit(progress)
            self.message_updated.emit(
                f'Quantifying tissues of images from path {index + 1}/{self.list.count()}: {image_path}')
            quantify_tissues = []
            if self.is_running:
                for row in range(self.tissue_list.rowCount()):
                    ...
                    if item.checkState() == Qt.Checked and (
                            not os.path.isfile(
                                os.path.join(classification_path, item.text() + '_count_analysis.csv'))):
