...
class CoREAM2_SegNet:
    ...
    def residual_encoder(self, input_features, num_filters, layer, filter_size=[3, 5, 7]):
        ...
        conv1_1 = layers.Conv2D(num_filters, filter_size[0], padding='same', activation=layers.LeakyReLU(),
                                dtype='float32', name=f'conv-e-{layer}_1_1')(input_features)
        conv1_2 = layers.Conv2D(num_filters, filter_size[1], padding='same', activation=layers.LeakyReLU(),
                                dtype='float32', name=f'conv-e-{layer}_1_2')(input_features)
        conv1_3 = layers.Conv2D(num_filters, filter_size[2], padding='same', activation=layers.LeakyReLU(),
                                dtype='float32', name=f'conv-e-{layer}_1_3')(input_features)
        ...
        conv2_1 = layers.Conv2D(filter_size[0], 1, padding='same', activation=layers.LeakyReLU(), dtype='float32',
                                name=f'conv-e-{layer}_2_1')(concat_12)
        conv2_2 = layers.Conv2D(filter_size[1], 1, padding='same', activation=layers.LeakyReLU(), dtype='float32',
                                name=f'conv-e-{layer}_2_2')(concat_23)
        conv2_3 = layers.Conv2D(filter_size[2], 1, padding='same', activation=layers.LeakyReLU(), dtype='float32',
                                name=f'conv-e-{layer}_2_3')(concat_13)

        concat_123 = layers.Concatenate(dtype='float32', name=f'conc-e-{layer}_123')([conv2_1, conv2_2, conv2_3])
        conv_fin = layers.Conv2D(num_filters, 1, padding='same', activation=layers.LeakyReLU(), dtype='float32',
                                 name=f'conv-e-{layer}_fin')(concat_123)
    ...
    def residual_decoder(self, input_features, num_filters, layer, filter_size=[3, 5, 7]):
        ...
        convt1_1 = layers.Conv2DTranspose(num_filters, filter_size[0], padding='same',
                                          activation=layers.LeakyReLU(), dtype='float32',
                                          name=f'convt-d-{layer}_1_1')(input_features)
        convt1_2 = layers.Conv2DTranspose(num_filters, filter_size[1], padding='same',
                                          activation=layers.LeakyReLU(), dtype='float32',
                                          name=f'convt-d-{layer}_1_2')(input_features)
        convt1_3 = layers.Conv2DTranspose(num_filters, filter_size[2], padding='same',
                                          activation=layers.LeakyReLU(), dtype='float32',
                                          name=f'convt-d-{layer}_1_3')(input_features)
        ...
        conv_fin = layers.Conv2D(num_filters, 1, padding='same', activation=layers.LeakyReLU(), dtype='float32',
                                 name=f'conv-d-{layer}_fin')(concat_123)
    ...
    def skip_concat(self, down_skip_connection, up_skip_connection, num_filters, layer):
        if (len(down_skip_connection) == 4):
            ...
            skip_connection_d = layers.Concatenate(dtype='float32', name=f'conc-down-d-{layer}')(
                [self.downsampler(down_skip_connection[0], 3), self.downsampler(down_skip_connection[1], 2),
                 self.downsampler(down_skip_connection[2], 1)])
        elif (len(down_skip_connection) == 2):
            skip_connection_d = layers.Concatenate(dtype='float32', name=f'conc-down-d-{layer}')(
                [self.downsampler(down_skip_connection[0], 2), self.downsampler(down_skip_connection[1], 1)])
        elif (len(down_skip_connection) == 1):
            skip_connection_d = layers.Concatenate(dtype='float32', name=f'conc-down-d-{layer}')(
                [self.downsampler(down_skip_connection[0], 1)])
        else:
            print("ERROR INITIALIZING DOWN SKIPS!!")
        ...
        if (len(up_skip_connection) == 4):
            skip_connection_u = layers.Concatenate(dtype='float32', name=f'conc-up-d-{layer}')(
                [self.upsampler(up_skip_connection[0], 3), self.upsampler(up_skip_connection[1], 2),
                 self.upsampler(up_skip_connection[2], 1), up_skip_connection[3]])
        elif (len(up_skip_connection) == 3):
            skip_connection_u = layers.Concatenate(dtype='float32', name=f'conc-up-d-{layer}')(
                [self.upsampler(up_skip_connection[0], 2), self.upsampler(up_skip_connection[1], 1),
                 up_skip_connection[2]])
        elif (len(up_skip_connection) == 2):
            skip_connection_u = layers.Concatenate(dtype='float32', name=f'conc-up-d-{layer}')(
                [self.upsampler(up_skip_connection[0], 1), up_skip_connection[1]])
        elif (len(up_skip_connection) == 1):
            skip_connection_u = layers.Concatenate(dtype='float32', name=f'conc-up-d-{layer}')(
                [up_skip_connection[0]])
        else:
            print("ERROR INITIALIZING UP SKIPS!!")

        concat_123 = layers.Concatenate(dtype='float32', name=f'conc-ud-{layer}_123')(
            [skip_connection_d, skip_connection_u])
        conv_fin = layers.Conv2D(num_filters, 1, padding='same', activation=layers.LeakyReLU(), dtype='float32',
                                 name=f'conv-s-{layer}_fin')(concat_123)
    ...
    def build_model(self):
        ...
        d_intout1 = layers.Conv2D(16, 3, padding='same', activation=layers.LeakyReLU(), name='feature_smoothen_1')(
            d4)
        d_intout2 = layers.Conv2D(16, 3, padding='same', activation=layers.LeakyReLU(), name='feature_smoothen_2')(
            d_intout1)
        d_out = layers.Conv2D(self.num_classes, kernel_size=1, padding='same', activation='softmax',
                              name='segmaps')(d_intout2)
