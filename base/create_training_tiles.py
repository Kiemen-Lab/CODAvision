...
def create_training_tiles(pthDL, numann0, ctlist0, create_new_tiles):
    ...
    for b, count in enumerate(annotation_composition):
        if annotation_composition[b] == 100:
            print(f' There are {np.round(count*max(count_annotations))} pixels of {classNames[b]}. This is the most common class.')
        else:
            print(
                f' There are {np.round(count*max(count_annotations))} pixels of {classNames[b]}, {int(annotation_composition[b])}% of the most common class.')
    ...
    if len(glob.glob(os.path.join(obg, 'HE*.jpg'))) >= ntrain:
        ...
        while len(glob.glob(os.path.join(obg, 'HE*.jpg'))) < ntrain:
            ...
            baseclass2 = np.sum(percann0[:, :, 1] == 1, axis=0)
            usedclass2 = np.sum(percann[:, :, 1] == 2, axis=0)
